...
from yx.teams.shared_task import SharedTask, SharedTaskStore
from yx.teams.mailbox import Mailbox, MailboxMessage, create_message
from yx.teams.registry import AgentNameRegistry
from yx.teams.backend_detect import BackendDetectionError, detect_backend, detect_pane_backend
from yx.teams.coordinator import (
    get_coordinator_system_prompt,
    get_coordinator_user_context,
    is_coordinator_mode,
    match_session_mode,
)
from yx.agents.tool_filter import (
...
class TestModels:
    ...
    def test_unique_team_name(self, tmp_dir):
        with patch("yx.teams.models.Path.home", return_value=Path(tmp_dir)):
            name1 = unique_team_name("my-team")
            assert name1 == "my-team"
            (Path(tmp_dir) / ".yx" / "teams" / "my-team").mkdir(parents=True)
...
class TestBackendDetect:
    ...
    def test_pane_iterm2_with_it2(self):
        ...
        with patch.dict(os.environ, env, clear=False):
            with patch("yx.teams.backend_detect.shutil.which") as mock_which:
    ...
    def test_pane_no_backend_falls_back_to_in_process(self):
        ...
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("TMUX", None)
            os.environ.pop("TERM_PROGRAM", None)
            with patch("yx.teams.backend_detect.shutil.which", return_value=None):
                result = detect_pane_backend()
                assert result == BackendType.IN_PROCESS
...
class TestConfigExtensions:
    def test_teammate_mode_defaults(self):
        from yx.config import AppConfig
    ...
    def test_load_config_with_team_fields(self, tmp_dir):
        from yx.config import load_config
...
class TestTranscript:
    ...
    def test_save_and_load(self, tmp_dir):
        ...
        from yx.teams.transcript import load_transcript, save_transcript
    ...
    def test_load_nonexistent(self, tmp_dir):
        from yx.teams.transcript import load_transcript
        with patch("yx.teams.models.Path.home", return_value=Path(tmp_dir)):
...
class TestAgentCoordinatorIntegration:
    def test_normal_prompt(self):
        from yx.prompts import build_system_prompt, IDENTITY_SECTION
    ...
    def test_coordinator_prompt(self):
        from yx.prompts import build_system_prompt
