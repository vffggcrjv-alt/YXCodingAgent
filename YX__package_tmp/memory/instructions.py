...
def load_instructions(project_root: str) -> str:
    ...
    发现顺序（低优先级在前，高优先级在后）：
    1. 用户全局：~/.yx/YX.md, ~/.yx/AGENTS.md
    2. 项目目录链：从 git root 到 workDir，每个目录的 YX.md 和 AGENTS.md
    3. workDir/.yx/INSTRUCTIONS.md（遗留格式）
    4. workDir/YX.local.md（本地覆盖）
    """
    ...
    # 3. 遗留格式
    _add(root / ".yx" / "INSTRUCTIONS.md")

    # 4. 本地覆盖
    _add(root / "YX.local.md")
