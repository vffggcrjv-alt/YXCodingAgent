...
from yx.conversation import ConversationManager, ToolResultBlock, ToolUseBlock
from yx.conversation import ThinkingBlock as ConvThinkingBlock
from yx.memory.auto_memory import MemoryManager
from yx.permissions import (
    Decision,
    PermissionChecker,
    PermissionMode,
)
from yx.hooks import HookContext, HookEngine, ToolRejectedError
from yx.hooks.engine import HookNotification
...
class Agent:
    def __init__(
        ...
        if memory_manager is not None:
            from yx.memory.consolidation import MemoryConsolidator
    ...
    def _transcript_path(self) -> str:
        if self.session_id:
            return str(Path(self.work_dir) / ".yx" / "sessions" / f"{self.session_id}.jsonl")
        return ""
    ...
    def _get_plan_path(self) -> Path:
        ...
        plans_dir = Path(self.work_dir) / ".yx" / "plans"
        ...
        if self.permission_checker:
            ...
            if decision.effect == "ask":
                ...
                if response == PermissionResponse.ALLOW_ALWAYS:
                    from yx.permissions.rules import Rule, extract_content
    ...
    def _maybe_persist_or_truncate(self, tool_use_id: str, text: str) -> str:
        from yx.context.manager import (
