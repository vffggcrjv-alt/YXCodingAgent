...
from yx.config import ConfigError, load_config
from yx.hooks import HookConfigError, HookEngine, load_hooks
from yx.permissions import PermissionMode


def main() -> None:
    # 先确保 .yx/ 目录存在，否则下面写 debug.log 会因目录不存在而崩溃
    Path(".yx").mkdir(parents=True, exist_ok=True)
    logging.basicConfig(
        level=logging.INFO,
        format="%(asctime)s %(name)s %(message)s",
        filename=".yx/debug.log",
        filemode="w",
    )

    parser = argparse.ArgumentParser(prog="yx", description="YX AI coding assistant")
    parser.add_argument(
        "--mode",
    ...
    if args.remote:
        from yx.remote import RemoteServer
    ...
    from yx.app import YXApp
    from yx.driver import NoAltScreenDriver
...
async def _run_prompt(config, permission_mode, hook_engine, prompt: str, output_format: str = "text") -> None:
    ...
    from yx.client import create_client, resolve_context_window
    from yx.conversation import ConversationManager
    from yx.memory.instructions import load_instructions
    ...
    from yx.tools import create_default_registry
    ...
    from yx.worktree import WorktreeManager
