...
from yx.commands import (
    CommandContext,
    CommandRegistry,
    complete,
    parse_command,
)
from yx.commands.completion import CompletionPopup
from yx.commands.handlers import register_all_commands
from yx.config import MCPServerConfig, ProviderConfig
from yx.hooks import HookContext, HookEngine, load_hooks
from yx.conversation import ConversationManager, Message
from yx.mcp import ConnectResult, MCPManager
from yx.memory import (
    MemoryManager,
    Session,
    SessionManager,
...
from yx.agents.task_manager import TaskManager
from yx.agents.trace import TraceManager
from yx.agents.notification import inject_task_notifications
from yx.commands.handlers.tasks import create_tasks_command
from yx.skills.executor import SkillExecutor
from yx.skills.loader import SkillLoader
from yx.commands.handlers.skill_register import register_skill_commands
from rich.text import Text as RichText
from textual.theme import Theme
from yx.cache import FileCache
from yx.tools import ToolRegistry, create_default_registry
from yx.tools.agent_tool import AgentTool
from yx.tools.ask_user import AskUserEvent, AskUserTool
from yx.tools.impl.tool_search import ToolSearchTool
from yx.tools.install_skill import InstallSkillTool
from yx.tools.load_skill import LoadSkill
from yx.worktree.cleanup import start_stale_cleanup_task
from yx.worktree.manager import WorktreeManager
from yx.commands.handlers.worktree import create_worktree_command
from yx.teammate_tree import TeammateTree
...
_SKIP_DIRS = {".git", "node_modules", ".venv", "__pycache__", ".yx", "build", ".gradle"}
...
class ChatInput(TextArea):
    ...
    def load_history(self, work_dir: str) -> None:
        self._history_file = Path(work_dir) / ".yx" / "history"
...
class YXApp(App):
    ...
    def __init__(
        ...
        from yx.config import SandboxAppConfig
    ...
    def _make_banner(model: str = "", work_dir: str = "") -> RichText:
        t = RichText()
        # Keep the logo as compact and legible as the original ASCII cat.
        logo_lines = [
            "  O  .  O  ",
            " o  ~~~  o ",
            " >       < ",
        ...
        details = ["YX v0.1.0", model, work_dir]
        for index, line in enumerate(logo_lines):
            t.append(line, style="bold #69D9FF")
            if index < len(details):
                t.append("   ")
                t.append(details[index], style="bold #087ce5" if index == 0 else "color(242)")
            t.append("\n")
        return t
    ...
    def _select_provider(self, provider: ProviderConfig) -> None:
        ...
        checker = PermissionChecker(
            ...
            rule_engine=RuleEngine(
                user_rules_path=home / ".yx" / "permissions.yaml",
                project_rules_path=Path(work_dir) / ".yx" / "permissions.yaml",
                local_rules_path=Path(work_dir) / ".yx" / "permissions.local.yaml",
            ),
        ...
        if self._sandbox_cfg.enabled:
            from yx.sandbox import SandboxConfig, create_sandbox
            os_sandbox = create_sandbox()
            if os_sandbox and os_sandbox.available():
                sandbox_config = SandboxConfig(
                    allow_write=[work_dir, "/tmp"],
                    deny_write=[
                        f"{work_dir}/.yx/config.yaml",
                        f"{work_dir}/.yx/permissions.local.yaml",
                    ],
        ...
        from yx.tools.exit_plan_mode import ExitPlanModeTool
        ...
        # --- Worktree 系统初始化 ---
        from yx.config import WorktreeConfig
        ...
        from yx.tools.enter_worktree import EnterWorktreeTool
        from yx.tools.exit_worktree import ExitWorktreeTool
        ...
        from yx.tools.team_create import TeamCreateTool
        from yx.tools.team_delete import TeamDeleteTool
        ...
        from yx.commands.handlers.trace import create_trace_command
        ...
        from yx.tools.synthetic_output import SyntheticOutputTool
    ...
    async def _show_plan_approval(self) -> None:
        from yx.plan_dialog import InlinePlanWidget
    ...
    def on_inline_plan_widget_responded(
        ...
        from yx.plan_dialog import InlinePlanWidget, PlanChoice
        from yx.prompts import build_plan_mode_exit_reminder
    ...
    async def _handle_askuser(self, event: AskUserEvent) -> None:
        from yx.askuser_dialog import InlineAskUserWidget
    ...
    async def _handle_permission_request(self, request: PermissionRequest) -> None:
        from yx.permission_dialog import InlinePermissionWidget
