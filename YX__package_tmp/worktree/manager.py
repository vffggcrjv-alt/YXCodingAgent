...
from yx.worktree.changes import (
    CleanupResult,
    Changes,
    count_worktree_changes,
    has_worktree_changes,
)
from yx.worktree.models import Worktree, WorktreeSession
from yx.worktree.session import load_worktree_session, save_worktree_session
from yx.worktree.setup import perform_post_creation_setup
...
class WorktreeManager:
    def __init__(
        ...
        self.worktree_dir = worktree_dir or str(
            Path(repo_root) / ".yx" / "worktrees"
        )
        self._yx_dir = Path(repo_root) / ".yx"
    ...
    async def enter(self, name: str) -> WorktreeSession:
        ...
        save_worktree_session(self._yx_dir, session)
        return session
    ...
    async def exit(
        ...
        save_worktree_session(self._yx_dir, None)
    ...
    def restore_session(self) -> WorktreeSession | None:
        session = load_worktree_session(self._yx_dir)
        if session is None:
            return None
